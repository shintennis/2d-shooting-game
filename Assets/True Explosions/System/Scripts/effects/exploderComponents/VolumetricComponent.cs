...
public class VolumetricComponent : ExploderComponent {
	...
	protected void emitNewParticles() {
		if ((Time.time - exploder.explosionTime) / duration < centerEmissionDuration) {
			ps.Emit(Mathf.Min((int) (centerEmission * Time.deltaTime), maxParticles - curCount));
		} else {
			ps.Emit(Mathf.Min((int) (emission * Time.deltaTime), maxParticles - curCount));
		}
		int nextCount = ps.GetParticles(particles);
